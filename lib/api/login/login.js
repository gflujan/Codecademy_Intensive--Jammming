...
const handler = async (event) => {
   const { API_SECRET_CLIENT_ID, SPOTIFY_AUTH_BASE_URL } = process.env;
   ...
   console.log("🚀--BLLR? ------------------------------------------------------");
   console.log("🚀--BLLR? -> f://SPOTIFY.js -> API_SECRET_CLIENT_ID ->", API_SECRET_CLIENT_ID);
   console.log("🚀--BLLR? ------------------------------------------------------");
   ...
   if (userAccessToken) {
      ...
      const loginWindow = window.open(
         `${SPOTIFY_AUTH_BASE_URL}?client_id=${API_SECRET_CLIENT_ID}&response_type=token&scope=${encodeURIComponent(scopes.join())}&redirect_uri=${encodeURIComponent(REDIRECT_URL)}`,
         "Spotify",
   ...
   if (accessTokenInUrl && expiresInUrl) {
      ...
      window.location = `${SPOTIFY_AUTH_BASE_URL}?client_id=${API_SECRET_CLIENT_ID}&response_type=token&scope=${scopes.join()}&redirect_uri=${REDIRECT_URL}`;
   }
};
